...
void hash(uint32_t *message_schedule, uint8_t* message_block, uint8_t* out)
{
	for (uint8_t i = 0; i < 16; i++)
	{
		message_schedule[i] = (message_block[i*4] << 24) | (message_block[(i*4)+1] << 16) | (message_block[(i*4)+2] << 8) | message_block[(i*4)+3];
	}
	...
	{
		uint32_t sig0 = rotate_right(w[0], 2) ^ rotate_right(w[0], 13) ^ rotate_right(w[0], 22);
		uint32_t sig1 = rotate_right(w[4], 6) ^ rotate_right(w[4], 11) ^ rotate_right(w[4], 25);
