/*
	 ...
	 - add function to tell how much mem will be needed for whole hash,
	   then allocate everything you will need all at start (on stack (probably))
	 - make (or find) a thing for printing hex vals
*/
...
uint32_t _rotate_right(uint32_t i, uint8_t d) { return (i >> d) | (i << (32 - d)); }
...
// NOTE:
// steps per chunk:
// - clear out schedule, copy chunk to beginning
// - do the run through to fill in the message schedule
// - do the working var run through
// - update working vars by adding h
// - if we're done, append to build hash, otherwise keep going
void hash(uint32_t* message_schedule, uint8_t* message_block, uint8_t num_chunks, uint8_t* out)
{
	uint32_t w[8];
	for (uint8_t i = 0; i < 8; i++)
	{
	...
	// {
		...
		{
			uint32_t sig0 = _rotate_right(message_schedule[word_i - 15], 7)
				^ _rotate_right(message_schedule[word_i - 15], 18)
				^ (message_schedule[word_i - 15] >> 3);
			uint32_t sig1 = _rotate_right(message_schedule[word_i - 2], 17)
				^ _rotate_right(message_schedule[word_i - 2], 19)
		...
		{
			uint32_t sig0 = _rotate_right(w[0], 2) ^ _rotate_right(w[0], 13) ^ _rotate_right(w[0], 22);
			uint32_t sig1 = _rotate_right(w[4], 6) ^ _rotate_right(w[4], 11) ^ _rotate_right(w[4], 25);
			uint32_t choice = (w[4] & w[5]) ^ ((~ w[4]) & w[6]);
			uint32_t maj = (w[0] & w[1]) ^ (w[0] & w[2]) ^ (w[1] & w[2]);
			uint32_t temp1 = w[7] + sig1 + choice + K[i] + message_schedule[i];
